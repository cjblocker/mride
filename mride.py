...
def get_mride_bus_stop_info(stop_id):
    ...
    try:
        stop_loc = requests.get(MRIDE_BUS_API_ENDPOINT.format(stop_id)).json()['bustime-response']['prd']
    except KeyError:
        print('Currently No Buses for that Stop')
        return []
...
def setup_notify(stop_id, route_id, minutes, display=False):
    ...
    if display: print_bus_info([bus for bus in stop_loc if int(bus['eta']) == time_left][0], prepend='\r', end="")
    ...
    try:
        while(1):
            ...
            for ii, bus in enumerate(stop_loc):
                stdscr.addstr(ii,0, "{:<3}| {:<30} | {:>2} min | {:%I:%M %p}".format(bus['short_name'],bus['name'].strip()[:30],bus['eta'], dt.now()+timedelta(minutes=int(bus['eta']))))
            stdscr.refresh()
            time.sleep(30)
...
def print_bus_info(bus, prepend='', end='\n'):
    print("{}{:<3}| {:<30} | {:>2} min | {:%I:%M %p}".format(prepend, bus['short_name'],bus['name'].strip()[:30],bus['eta'], dt.now()+timedelta(minutes=int(bus['eta']))), end=end)
...
def parse_arguments():
    ...
    parser.add_argument('-t','--ticker', action="store_true",
                        ...
                        --notify or --display.')
    parser.add_argument('-r','--route', type=str, default='*', metavar='route_id',
...
def main(stop_id=WORK_STOP_ID, route_id=22, do_print=True, notify_minutes=0, do_ticker=False):
    if do_ticker:
